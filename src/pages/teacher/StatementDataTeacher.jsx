...
const StatementDataTeacher = () => {

    const userId = useSelector(state => state.teacher?.teacher?.id)
    ...
    const getAllVedimostByTeacher = (eduId) => {
        if (!eduId) return;

        setLoad(true)
        axios.get(`${BASE_URL}/vedimost/getVedimostByTeacherId/${userId}?educationYearId=${eduId}`, getHeaders())
            .then(res => {
    ...
    useEffect(() => {
        getAllVedimostByTeacher(eduYearValue)
    }, [eduYearValue]);
    const columns = [
        {
            field: 'count',
...
const Container = styled.div`
    width: 100%;
    padding: 1rem;`;
export default StatementDataTeacher;