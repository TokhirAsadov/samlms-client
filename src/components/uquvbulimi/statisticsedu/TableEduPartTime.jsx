    ...
    const courseLevel = [
        ...
        },
        {
            ...
            faculty: [
                ...
                {
                    name: 'BANKING (Memorandums with banks)'
                },
            ]
        },
        {
            ...
            <MainBox>
                ...
                <TableScrollBox>
                    <TableBox>
                        ...
                        {courseLevel.map((item, key) => (
                            <>
                                <tr key={key} style={{textAlign: "center",}}>
                                    <th style={{textAlign: "center", background: "#007eb0"}}> {item.level === 1 && "I"}
                                        {item.level === 2 && "II"}
                                        {item.level === 3 && "III"}
                                        {item.level === 4 && "IV"}</th>
                                </tr>
                                {item.faculty.map((faculty, key) => (
                                    <tr key={key}>
                                        <th>{faculty.name}</th>
                                        {
                                            weeks.map((day, key) => {
                                                ...
                                                return <td key={key}
                                                           className={`itemday ${allData.find(findItem => findItem.weeksOrder === day && findItem?.faculty === weekinfo?.faculty && findItem?.level == weekinfo?.level) ? "selected" : ""}`}
...
const Container = styled.div`
    ...
    width: 100%;
`
export default TableEduPartTime;