...
const TimeTableForUquvBulimDashboard = ({index, data, statistics,objWeek}) => {
                ...
                check ?
                    <Table>
                        ...
                        <TableBody>
                            ...
                            {
                                weekdays?.filter(i => objWeek?.weekday ? i.day === objWeek?.weekday : i.day === new Date().getDay())?.map(
                                    (weekday, index) => {
                                        ...
                                        return (
                                            <TableRow key={index}>
                                                <TableItem flex={1}>
                                                    <DuringTime>{data && data[0]?.room?.length === 7 ? data[0]?.room : data[0]?.room?.substring(0, 5)}</DuringTime>
                                                </TableItem>

                                                {
                                                    hours?.map((hour, index) => {
                                                        ...
                                                        let timesTouch = statistics?.filter(item => item?.section === hour?.number)?.sort(function (o1, o2) {
                                                        ...
                                                        return hourItem && sorted ?
                                                            (
                                                                <TableItem flex={2} key={index}
                                                                           bg={(timesTouch === undefined || timesTouch?.length === 0) && ((objWeek?.weekday <= new Date().getDay() && (hour?.start < new Date() || tim2 < new Date())) || objWeek.weekNumber < thisWeek || objWeek.year < new Date().getFullYear())}>
                                                                    <TableItemWrapper>
                                                                        <LessonName
                                                                            fontSize={hourItem?.lessonName?.length > 18 ? "8px" : "7px"}>{hourItem.lessonName}</LessonName>

                                                                        {
                                                                            timesTouch && timesTouch?.length !== 0 &&
                                                                            <TimeTouch
                                                                                color={new Date(hour?.start).setDate(
                                                                        ...
                                                                        {
                                                                            (timesTouch === undefined || timesTouch?.length === 0) && ((objWeek.weekday<= new Date().getDay() && (hour?.start < new Date() || tim2 < new Date())) || objWeek?.weekNumber < thisWeek) &&
                                                                            <TimeTouch color={false}>
                                                                                {/*{moment(new Date() - hour?.start).format("mm:ss")}*/}
                                                                                <MdRemoveCircleOutline
                                                                                    size={12}/> {" "} 50:00
                                                                            </TimeTouch>
                                                                        }
                                                                        ...
                                                                        <LessonGroups
                                                                            fontSize={hourItem?.teacherName?.length > 1 ? "8px" : "7px"}>
                                                                            {
...
export default TimeTableForUquvBulimDashboard;