...
const StatementDataDean = () => {
    ...
    const getFacultyAll = () => {
      axios.get(`${BASE_URL}/faculty/getDirectionsOfFaculty`,getHeaders())
          .then(res => {
              console.log(res.data)
              setOptionDirection(res.data?.obj)
              setDirectionValue(res.data?.obj[0]?.id)
          })
          .catch(err => {
              console.log(err)
          })
    ...
    useEffect(() => {
        if (eduYearValue && directionValue) {
